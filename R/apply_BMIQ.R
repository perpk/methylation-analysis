...
plot_BMIQ <- function(context = NULL) {
  ...
  beta_before <- readRDS(file.path(context$paths$results, "beta_matrix_no_outliers.rds"))
...
.plot_BMIQ_comparison <- function(beta_before, beta_after, platform, context = NULL) {
  library(ggplot2)
  library(patchwork)
