apply_BMIQ <- function(context = NULL,
  ...
  for (i in 1:ncol(beta_matrix)) {
    ...
    tryCatch({
      bmiq_result <- BMIQ(beta.v = beta.v,
                          design.v = design.v,
                          nfit = 10000,
                          plots = FALSE,
                          pri = TRUE)
...
.plot_BMIQ_comparison <- function(beta_before, beta_after) {
  ...
  plot_data <- data.frame(
    ...
    ProbeType = rep(.get_probe_types(rownames(beta_before)),
                    times = ncol(beta_before) * 2)
  )

  p1 <- ggplot(plot_data, aes(x = Beta, color = Method)) +
    geom_density(alpha = 0.5) +
    labs(title = "Overall Beta Distribution",
  ...
  p2 <- ggplot(plot_data, aes(x = Beta, color = Method)) +
    ...
    labs(title = "Beta Distribution by Probe Type",
         x = "Beta Value", y = "Density") +
    theme_minimal()
...
.get_probe_types <- function(probe_names) {
...
.get_probe_design_vector <- function(probe_names) {
  ...
  if (length(matched_probes) < length(probe_names)) {
    warning(paste(length(probe_names) - length(matched_probes),
