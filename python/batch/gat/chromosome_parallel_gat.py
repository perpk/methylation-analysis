...
class ChromosomeParallelGAT(nn.Module):
    def __init__(self, num_node_classes=7, chr_embed_dim=16, cell_prop_dim=6):
        ...
        # 1. Widened capacity for 450k array + 20% edge dropout
        self.gat1 = GATv2Conv(
            in_channels=9, 
            out_channels=16, 
            heads=4, 
        ...
        # gat1 output is 16 channels * 4 heads = 64
        self.gat2 = GATv2Conv(
            in_channels=64, 
            out_channels=chr_embed_dim, 
            heads=1, 
            concat=True, 
            add_self_loops=False, 
            dropout=0.2
    ...
    def forward(self, batched_chrs, u_cells):
        ...
        for c_idx in range(22):
            ...
            # 2. 30% node dropout to protect raw CpG features
            node_feat = F.dropout(node_feat, p=0.3, training=self.training)
            
            # 3. ELU activation to prevent dead neurons in the heads
            h = F.elu(self.gat1(node_feat, batch.edge_index))
            
            # 4. 30% node dropout on hidden embeddings
        ...
        # 5. Final 30% dropout before classification
