...
normalize_extract_methylset(project_context)
...
source('R/principal_component_analysis.R')
pca_df <- principal_component_analysis(project_context, beta_matrix_filename = "beta_matrix_bmiq.rds", targets_filename = "targets_clean.rds", col_map, keys, 5)
...
data_to_clean <- list(
  beta_matrix_data = list(
    filepath = file.path(project_context$paths$processed, "beta_matrix_bmiq.rds"),
    clean_filename = file.path(project_context$paths$processed, "beta_matrix_s_mismatch.rds")
  ),
  m_values_data = list(
    filepath = file.path(project_context$paths$processed, "m_values_bmiq.rds"),
...
clean_data(samples_to_remove = samples_to_remove, data_to_clean = targets_to_clean)
gc(full = T, verbose = T)

source('R/cell_cnt_estimate.R')
cell_cnt_estimate(context = project_context, rg_set_filename = "rg_set_s_mismatch.rds", targets_filename = "targets_s_mismatch.rds")
